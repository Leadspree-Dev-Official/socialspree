...
export function AuthGate({ onCancel, onDemoLogin, onAuthenticated }: AuthGateProps) {
  ...
  const handleOAuthLogin = async (strategy: 'oauth_google' | 'oauth_facebook') => {
    ...
    try {
      ...
      setAuthError(msg || `OAuth strategy '${strategy}' requires enabling in Clerk Dashboard.`);
    }
  };
  ...
  const handleClerkSubmit = async (e: React.FormEvent) => {
    ...
    if (!isSignInLoaded || !signIn) {
      setAuthError('Clerk authentication service is initializing. Please try again.');
      return;
    }
    ...
    try {
      ...
      if (result.status === 'complete') {
        ...
        setAuthError(`Clerk authentication status: ${result.status}. Verification required.`);
      }
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFF] flex flex-col items-center justify-center p-5 font-['Inter'] space-y-6">
      <div className="w-full max-w-md flex flex-col items-center">
        ...
        {/* CLERK AUTHENTICATION ENGINE CARD */}
          ...
          <form onSubmit={handleClerkSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full p-3 border border-slate-300 rounded-xl text-xs font-mono focus:ring-2 focus:ring-purple-500 focus:outline-none"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full p-3 border border-slate-300 rounded-xl text-xs focus:ring-2 focus:ring-purple-500 focus:outline-none"
              />
            </div>
            ...
            >
              {authLoading ? (
                <>
                  ...
                  <span>Authenticating via Clerk API...</span>
                </>
              ) : (
                <>
