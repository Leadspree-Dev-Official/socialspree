...
Deno.serve(async req => {
  ...
  try {
    ...
    if (typeof prompt !== 'string' || !prompt.trim()) return json({ error: 'Prompt required' }, 400, req);
    if (prompt.length > 8000) return json({ error: 'Prompt too long' }, 422, req);
    if (!['caption', 'hashtag', 'description'].includes(mode)) return json({ error: 'Unsupported AI mode' }, 422, req);
    ...
    if (reserveError) return json({ error: 'Insufficient AI credits' }, 402, req);
    ...
    if (!apiKey) {
      ...
      return json({ error: 'AI provider is not configured' }, 503, req);
    }
    ...
    try {
      ...
      return json({ content, creditsRemaining: remaining }, 200, req);
    } catch (providerError) {
      ...
      return json({ error: providerError instanceof Error ? providerError.message : 'AI provider request failed' }, 502, req);
    }
  } catch (e) {
