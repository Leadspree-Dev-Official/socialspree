...
Deno.serve(async req => {
  ...
  try {
    ...
    if (!tenantId || !provider) return json({ error: 'tenantId and provider are required' }, 400, req);
    if (remove) {
      ...
      return json({ removed: true }, 200, req);
    }
    const clearSecret = generate ? `ssp_${crypto.randomUUID().replaceAll('-', '')}${crypto.randomUUID().replaceAll('-', '')}` : secret;
    if (typeof clearSecret !== 'string' || clearSecret.length < 8) return json({ error: 'A valid secret is required' }, 400, req);
    ...
    if (error) throw error;
    return json({ saved: true, masked: `••••${clearSecret.slice(-4)}`, validation, generatedSecret: generate ? clearSecret : undefined }, 200, req);
