...
Deno.serve(async req => {
  ...
  try {
    const { db, profile } = await actor(req); const body = await req.json().catch(() => ({}));
    ...
    for (const post of posts ?? []) {
      ...
      const { data } = await zernioClient(key).analytics.getAnalytics({ query: { postId: post.zernio_post_id } });
      ...
      await db.from('analytics_snapshots').upsert(row, { onConflict: 'tenant_id,zernio_post_id' }); snapshots.push(row);
    }
