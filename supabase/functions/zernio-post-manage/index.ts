...
Deno.serve(async req => {
  ...
  try {
    ...
    if (action === 'get') {
      ...
      if (!postRow.zernio_post_id) return json({ post: postRow, remoteStatus: postRow.status });

      const { data: remoteData } = await client.posts.getPost({ path: { id: postRow.zernio_post_id } } as any);
      return json({ post: postRow, remotePost: remoteData });
    }
    ...
    return json({ error: n.message, code: n.code }, 400);
  }
});
