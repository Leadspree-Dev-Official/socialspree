-- Grant permissions and fix execution rights for ensure_clerk_profile function
...
AS $$
DECLARE
  subject TEXT := COALESCE(auth.jwt() ->> 'sub', (SELECT id FROM public.profiles WHERE lower(email) = lower(trim(p_email)) LIMIT 1));
  jwt_email TEXT := lower(COALESCE(auth.jwt() ->> 'email', trim(p_email)));
...
-- Grant execution permissions broadly so RPC call succeeds under both authenticated and anon roles
