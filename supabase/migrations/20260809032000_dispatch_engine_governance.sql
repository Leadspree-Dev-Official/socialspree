-- Migration: Dispatch Engine Governance & Multi-Engine / Agency Slot Bundles

ALTER TABLE public.tenants 
ADD COLUMN IF NOT EXISTS dispatch_engine TEXT NOT NULL DEFAULT 'dual',
ADD COLUMN IF NOT EXISTS enabled_engines TEXT[] NOT NULL DEFAULT ARRAY['zenith', 'coresync'],
ADD COLUMN IF NOT EXISTS agency_slot_bundle_limit INT NOT NULL DEFAULT 10,
ADD COLUMN IF NOT EXISTS agency_max_brands INT NOT NULL DEFAULT 10;

INSERT INTO public.system_settings (key, value)
VALUES 
  ('default_dispatch_engine', 'dual'),
  ('coresync_all_channels_enabled', 'true')
ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value;

-- Update ensure_clerk_profile function with multi-engine defaults
...
BEGIN
  ...
  -- Provision default tenant with engine defaults
  IF result.tenant_id IS NULL THEN
    ...
    BEGIN
      INSERT INTO public.tenants (
        ...
        dispatch_engine, enabled_engines
      )
      VALUES (
        new_tenant_id, 
